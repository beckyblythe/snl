...
def plot_histograms(node, ISI, ISI_quiet, ISI_burst, Min_Volt, time_above, time_down, time_up):
    ...
    plt.subplot(1,3,1)
    plt.title(str(ISI.shape[0]) + ' ISIs\nMean: ' + str (round(ISI.mean(),2)) + 's')
    plt.xlabel('ISI (s)')
    plt.ylabel('Distribution of ISIs')
    plt.xlim((0,10))
    plt.hist(ISI, normed = True, bins = 50,range = ((0,10)),  alpha = .5)
#    plt.axvline(ISI.mean(), color = 'b')
    plt.subplot(1,3,2)
    plt.title(str(ISI_quiet.shape[0]) + ' quiet ISIs\nMean: ' + str (round(ISI_quiet.mean(),2)) + 's')
    plt.xlabel('ISI (s)')
    plt.hist(ISI_quiet, normed = True, bins = 50,range = ((0,10)) , alpha = .5)
#    plt.axvline(ISI_quiet.mean(), color = 'b')
    plt.xlim((0,10))
    plt.subplot(1,3,3)
    plt.title(str(ISI_burst.shape[0]) + ' burst ISIs\nMean: ' + str (round(ISI_burst.mean(),2)) + 's')
    plt.xlabel('ISI (s)')
    plt.hist(ISI_burst,normed = True, bins = 50, range = ((0,1)), alpha = .5)
#    plt.axvline(ISI_burst.mean(), color = 'b')
    plt.xlim((0,1))
#    plt.subplot(2,3,4)
